...
class TreeTools:
    ...
    def TrackMainHaloProgenitor(self,MainSubhaloID,SnapshotNr):
        ...
        sub_VelDisp=self.SubhaloVelDisp[istart:ifinish]
        ...
        while first_prog[idx]!=-1:     # Loop over list until we hit the root
            ...
            vel_disp=np.append(vel_disp,sub_VelDisp[idx])
        ...
        if tree_length>10:
            ...
            mass_z03=np.interp(0.3, redshift, mass)
#             mass_z03=np.interp(-0.3, np.flip(redshift), np.flip(mass))
#             alpha=(np.log10(mass[0])-np.log10(mass_z03))/0.3
            alpha=(mass[0]-mass_z03)/0.3
        
        return redshift,mass,m200,vel_disp,group_number,subhalo_number,zform,alpha
